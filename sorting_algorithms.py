...
def load_generated_datasets(dataset_dir=DATASET_DIR):
    """Load datasets generated by generate_synthetic_data.py from CSV files."""
    ...
    for entry in manifest.get("datasets", []):
        size = int(entry["size"])
        csv_path = dataset_dir / entry["file"]
        if not csv_path.exists():
            raise FileNotFoundError(
                f"Dataset file listed in manifest is missing: {csv_path}"
            )
        ...
        with csv_path.open("r", newline="", encoding="utf-8") as handle:
            ...
            for row in reader:
                rows.append((int(row["Product_ID"]), int(row["Price_ETB"])))

        datasets[size] = rows

    if not datasets:
        raise ValueError(
            f"No datasets found in {manifest_path}. "
            "Regenerate data using generate_synthetic_data.py."
        )
...
def main():
    ...
    plot_results(sizes, results, save_path="sorting_comparison.png")
